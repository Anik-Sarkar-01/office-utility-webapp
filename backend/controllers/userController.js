...
const newUser = async (req, res) => {
  ...
  const { email, password, name, position, isSuperUser, address, dateOfBirth, phone, joiningDate } = req.body;

  try {
    const existingUser = await userModel.findOne({ email });
    if (existingUser) return res.status(409).json({ message: "Email already exists", success: false });
